...
```sh
cargo install --locked x07-wasm --version 0.2.7
```
...
The device-capability sidecar now carries the Forge/Tactics M0 fields consumed by `std-web-ui@0.2.5` and `x07-device-host`: `audio.playback`, `haptics.present`, `clipboard.read_text`, `clipboard.write_text`, `files.pick_multiple`, `files.save`, `files.drop`, and `share.present`.

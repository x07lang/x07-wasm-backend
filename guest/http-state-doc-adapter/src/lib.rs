...
wit_bindgen::generate!({
    world: "x07:http-state-doc-adapter/proxy-with-solve@0.1.0",
    path: [
        ...
        "../../wit/x07/solve/0.1.0",
        "../../wit/x07/http_state_doc_adapter/0.1.0",
    ],
    generate_all,
});
...
struct HttpStateDocAdapter;

impl exports::wasi::http::incoming_handler::Guest for HttpStateDocAdapter {
    fn handle(request: wasi::http::types::IncomingRequest, response_out: wasi::http::types::ResponseOutparam) {
        ...
        let mut state_doc_json = Vec::from(b"null".as_slice());
        for (k, v) in headers_resource.entries().into_iter() {
            if k.as_str().eq_ignore_ascii_case(X07_STATE_DOC_REQUEST_HEADER) {
                state_doc_json = base64::engine::general_purpose::STANDARD
                    .decode(&v)
        ...
        let env_bytes = backend_request_bytes(
            request_id,
            &method,
            &path,
            &query,
        ...
        let resp = parse_backend_response(&resp_bytes);
        if resp.response.request_id != request_id {
            panic_invalid_response_envelope();
        }

        let mut header_entries = resp
            .response
            .headers
            .iter()
            .map(|(k, v)| (k.clone(), v.as_bytes().to_vec()))
            .collect::<Vec<_>>();
        let next_state_doc_b64 =
            base64::engine::general_purpose::STANDARD.encode(resp.state_json.as_slice());
        header_entries.push((
            String::from(X07_STATE_DOC_RESPONSE_HEADER),
            next_state_doc_b64.into_bytes(),
        ));
...
fn backend_request_bytes(
    ...
    let request_json = request_envelope_bytes(id, method, path, query, headers, body_bytes_len, body_b64);
    let mut out = Vec::new();
    out.extend_from_slice(br#"{"schema_version":"x07.app.backend.request@0.1.0","request":"#);
    out.extend_from_slice(request_json.as_slice());
    out.extend_from_slice(br#","state":"#);
    out.extend_from_slice(state_doc_json);
    out.push(b'}');
    out
}
...
struct BackendResponseDoc {
    response: HttpResponseEnvelope,
    state_json: Vec<u8>,
}

fn parse_backend_response(bytes: &[u8]) -> BackendResponseDoc {
    ...
    loop {
        ...
        match key.as_str() {
            ...
            "response" => {
                let start = cur.i;
                cur.skip_value();
                response_bytes = Some(cur.bytes[start..cur.i].to_vec());
            }
            "state" => {
                let start = cur.i;
                cur.skip_value();
                state_bytes = Some(cur.bytes[start..cur.i].to_vec());
            }
            _ => cur.skip_value(),
        }
    ...
    if schema_version != "x07.app.backend.response@0.1.0" {
        panic_invalid_response_envelope();
    }
    let Some(response_bytes) = response_bytes else {
        panic_invalid_response_envelope();
    };
    let Some(state_json) = state_bytes else {
        panic_invalid_response_envelope();
    };

    BackendResponseDoc {
        response: parse_response_envelope(response_bytes.as_slice()),
        state_json,
    }
}

...
use crate::cli::{MachineArgs, Scope, SloValidateArgs};
use crate::diag::{Diagnostic, Severity, Stage};
use crate::report;
use crate::schema::SchemaStore;
use crate::util;

pub fn cmd_slo_validate(
    raw_argv: &[OsString],
    scope: Scope,
    machine: &MachineArgs,
    args: SloValidateArgs,
) -> Result<u8> {
    ...
    let (profile_status, compatibility_hash) =
        validate_slo_profile(&store, &args.profile, &mut meta, &mut diagnostics)?;

    let ok = diagnostics.iter().all(|d| d.severity != Severity::Error);
    let exit_code = report::exit_code::exit_code_for_diagnostics(&diagnostics);

    let report_doc = json!({
      "schema_version": "x07.wasm.slo.validate.report@0.1.0",
      "command": "x07-wasm.slo.validate",
      "ok": ok,
      "exit_code": exit_code,
      "diagnostics": diagnostics,
      "meta": meta,
      "result": {
        "slo_profile": profile_status,
...
fn validate_slo_profile(
    ...
    let bytes = match std::fs::read(path) {
        ...
        Err(err) => {
            diagnostics.push(Diagnostic::new(
                ...
                Severity::Error,
                Stage::Parse,
                format!("failed to read SLO profile {}: {err}", path.display()),
            ));
            let status = json!({
    ...
    let doc_json: Value = match serde_json::from_slice(&bytes) {
        ...
        Err(err) => {
            diagnostics.push(Diagnostic::new(
                ...
                Severity::Error,
                Stage::Parse,
                format!("SLO profile JSON invalid: {err}"),
            ));
            let status = json!({
              "path": digest.path,
              "ok": false,
    ...
    let schema_diags =
        store.validate("https://x07.io/spec/x07-slo.profile.schema.json", &doc_json)?;
    ...
    let compat_doc = json!({ "slo_profile_sha256": digest.sha256 });
